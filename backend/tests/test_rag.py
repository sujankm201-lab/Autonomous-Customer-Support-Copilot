...
class TestDocumentLoader:
    ...
    def test_load_txt_file(self):
        ...
        with tempfile.NamedTemporaryFile(mode='w', suffix='.txt', delete=False) as f:
            ...
            content = loader.load_txt(f.name)
    ...
    def test_load_directory(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            # Create test files
            (Path(tmpdir) / "file1.txt").write_text("Document 1")
            (Path(tmpdir) / "file2.txt").write_text("Document 2")
...
class TestTextChunker:
    ...
    def test_chunk_text_basic(self):
        ...
        assert all(len(c.text) <= 60 for c in chunks)  # Allow some margin
    ...
    def test_chunk_by_sentences(self):
        ...
        chunks = TextChunker.chunk_by_sentences(text, sentences_per_chunk=2)
...
class TestEmbeddingsService:
    ...
    def test_embed_text(self, embeddings_service):
        ...
        embedding = embeddings_service.embed_text("This is a test sentence.")
        
        assert embedding is not None
        assert len(embedding) > 0
        assert embeddings_service.get_embedding_dimension() == len(embedding)

    def test_embed_texts_batch(self, embeddings_service):
        ...
        assert all(len(e) == embeddings_service.get_embedding_dimension() for e in embeddings)

    def test_compute_similarity(self, embeddings_service):
        ...
        sim_12 = embeddings_service.compute_similarity(emb1, emb2)
        sim_13 = embeddings_service.compute_similarity(emb1, emb3)
...
class TestLLMService:
    ...
    def test_llm_generate(self):
        ...
        llm = LLMService()
        response = llm.generate("Complete this sentence: The answer is")
...
class TestVectorDatabase:
    ...
    def test_vector_db_initialization(self, temp_db):
    ...
    def test_add_and_query_documents(self, temp_db):
        ...
        chunks = [
            Chunk(text="Machine learning is a subset of AI.", chunk_index=0, metadata={}),
            Chunk(text="Deep learning uses neural networks.", chunk_index=1, metadata={}),
        ]
        ...
        # Add to database
        num_added = db.add_documents(chunks, embeddings)
        assert num_added == 2
        
        # Query
        query_embedding = embeddings_service.embed_text("What is AI?")
...
class TestRAGPipeline:
    ...
    def test_rag_indexing(self, temp_rag):
        ...
        documents = [
            ...
            Document(
                content="Machine learning is about training models.",
                metadata={"source": "doc2.txt"}
            ),
        ]
        
        num_indexed = temp_rag.index_documents(documents)
        assert num_indexed > 0

    def test_rag_query(self, temp_rag):
        """Test RAG query."""
        ...
        temp_rag.index_documents(documents)
        
        # Query
        result = temp_rag.query("What is Python?")
    ...
    def test_rag_confidence_scoring(self, temp_rag):
        """Test confidence scoring."""
        query = "What is AI?"
        answer = "Artificial Intelligence is the simulation of human intelligence."
        context = "AI refers to the capability of a computer or machine to mimic cognitive functions."
        
        confidence = temp_rag._calculate_confidence(query, answer, context)
...
class TestKnowledgeBaseManager:
    ...
    def temp_kb(self):
        ...
        kb = KnowledgeBaseManager(rag, tmpdir)
        yield kb
        # Cleanup
        if Path(tmpdir).exists():
            shutil.rmtree(tmpdir)

    def test_kb_health_check(self, temp_kb):
        """Test knowledge base health check."""
        is_healthy = temp_kb.health_check()
        assert is_healthy

    def test_kb_stats(self, temp_kb):
        """Test getting knowledge base stats."""
        stats = temp_kb.get_knowledge_base_stats()
    ...
    def test_end_to_end_rag_workflow(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            documents = [
                ...
                Document(
                    content="Django is a high-level web framework for web development.",
